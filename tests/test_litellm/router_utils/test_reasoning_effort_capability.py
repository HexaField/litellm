...
class TestDeclaredEffortList:
    """reasoning_effort_levels is what the CATALOG declares about one deployment. It is a different
    fact from ModelGroupInfo.supported_reasoning_efforts, which is what the group COMPUTED across
    its deployments, and test_router.py pins that the computed name is never seeded from model_info.
    Keeping the two names apart is what lets both invariants hold at once."""
...
class TestKimiK3AdvertisesItsDocumentedLevels:
    ...
    def test_a_passthrough_entry_advertises_the_models_own_levels(self, local_model_cost_map, model_key):
        ...
        and each of these providers forwards the level unchanged. Without the declaration every one
        of these entries carries supports_reasoning alone and resolves to unknown, which makes the
        dashboard fall back to a capability-blind list that deliberately omits max."""
    ...
    def test_the_perplexity_entry_advertises_the_wider_set_it_maps_down(self, local_model_cost_map):
        """Perplexity's Agent API takes a six-value enum for this model and maps it onto the model's
        own tiers internally, so the entry describing THAT deployment is legitimately wider than the
        ones describing a passthrough. One blanket list could not say both, which is the whole
        reason this key is per entry."""
